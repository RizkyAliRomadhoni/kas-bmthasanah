...
<div class="card shadow-sm border-0 rounded-4 mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('kas.index') }}" class="d-flex flex-wrap gap-3 align-items-center">
            <label for="bulan" class="fw-semibold text-gray-700">📅 Pilih Bulan:</label>
            <input type="month" name="bulan" id="bulan" value="{{ request('bulan') }}"
                class="form-control border-gray-300 rounded-3 shadow-sm w-auto">

            <label for="akun" class="fw-semibold text-gray-700">💼 Akun:</label>
        ...
        <div class="row text-center">
            <div class="col-md-4">
                <div class="p-3 bg-success bg-opacity-10 rounded-3 shadow-sm">
                    <h6 class="fw-bold text-white text-success">Pemasukan</h6>
                    <h4 class="fw-bold text-white">Rp{{ number_format($totalMasuk, 0, ',', '.') }}</h4>
                </div>
            </div>

            <div class="col-md-4">
                <div class="p-3 bg-danger bg-opacity-10 rounded-3 shadow-sm">
                    <h6 class="fw-bold text-white text-danger">Pengeluaran</h6>
                    <h4 class="fw-bold text-white">Rp{{ number_format($totalKeluar, 0, ',', '.') }}</h4>
                </div>
            </div>

            <div class="col-md-4">
                <div class="p-3 bg-primary bg-opacity-10 rounded-3 shadow-sm">
                    <h6 class="fw-bold text-white text-primary">Saldo Akhir</h6>
                    <h4 class="text-white fw-bold">Rp{{ number_format($saldoRingkasan, 0, ',', '.') }}</h4>
                </div>
            </div>
        </div>
    </div>
...
{{-- TABEL DATA KAS --}}
                                    ...
                                    <td class="text-center">
                                        ...
                                        <form action="{{ route('kas.destroy', $item->id) }}" method="POST" class="d-inline">
                                            ...
                                            <button onclick="return confirm('Hapus data ini?')" 
                                                class="btn btn-sm btn-danger px-2 py-1">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
        ...
        <a href="{{ route('kas.resetSaldo') }}" class="btn btn-danger"
